...
```Q#
BlockEncodingByLCU<'T, 'S> (statePreparation : ('T => Unit is Adj + Ctl), selector : (('T, 'S) => Unit is Adj + Ctl)) : (('T, 'S) => Unit is Adj + Ctl)
```


## Input

### statePreparation : 'T => Unit Adj + Ctl

A unitary $P$ that prepares some target state.


### selector : ('T,'S) => Unit Adj + Ctl
