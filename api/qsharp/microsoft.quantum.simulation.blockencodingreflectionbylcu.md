...
```Q#
BlockEncodingReflectionByLCU (statePreparation : (Qubit[] => Unit is Adj + Ctl), selector : ((Qubit[], Qubit[]) => Unit is Adj + Ctl)) : Microsoft.Quantum.Simulation.BlockEncodingReflection
```


## Input

### statePreparation : Qubit[] => Unit Adj + Ctl

A unitary $P$ that prepares some target state.


### selector : (Qubit[],Qubit[]) => Unit Adj + Ctl
