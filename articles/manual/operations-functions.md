...
Similarly, `(Qubit => Unit is Ctl)` denotes that an operation of that type supports the `Controlled` functor. We explore this further when we discuss [types in Q#] (xref:microsoft.quantum.manual.type-model) more generally.
