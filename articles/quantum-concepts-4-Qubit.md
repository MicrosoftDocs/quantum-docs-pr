...
%Qubits are quantum generalizations of bits and are the fundamental memory element in a quantum computer.
...
Any two-dimensional column vector of real or complex numbers with norm $1$ represents a qubit. Thus $\begin{bmatrix} \alpha \\ \beta \end{bmatrix}$ represents a qubit if $\alpha$ and $\beta$ are complex numbers satisfying $|\alpha|^2 + |\beta|^2 = 1$. Some examples of valid quantum state vectors representing qubits include $\begin{bmatrix} 1 \\ 0 \end{bmatrix}, \begin{bmatrix} 0 \\ 1 \end{bmatrix}, \begin{bmatrix} \frac{1}{\sqrt{2}} \\ \frac{1}{\sqrt{2}} \end{bmatrix}, \begin{bmatrix} \frac{1}{\sqrt{2}} \\ \frac{-1}{\sqrt{2}} \end{bmatrix},$ and $\begin{bmatrix} \frac{1}{\sqrt{2}} \\ \frac{i}{\sqrt{2}} \end{bmatrix}$. 

The quantum state vectors $\begin{bmatrix} 1 \\ 0 \end{bmatrix}$ and $\begin{bmatrix} 0 \\ 1 \end{bmatrix}$ take a special role.  These two vectors form a basis for the vector space that describes the qubit's state.  This means that any quantum state vector can be written as a sum of these basis vectors. Specifically, for vector $\begin{bmatrix} x \\ y \end{bmatrix}$ can be written as $x \begin{bmatrix} 1 \\0 \end{bmatrix} + y \begin{bmatrix} 0 \\ 1 \end{bmatrix}$.  While any rotation of these vectors would serve as a perfectly valid basis for the qubit we choose to privilege this one by calling it the computational basis.  This appellation is earned because of the way we choose two interpret these specific vectors.  We take these two quantum states to correspond to the two states a classical bit can take.  The standard convention is to choose
\begin{equation}
...
\begin{figure}[h!]
\includegraphics[width=0.4\linewidth]{bloch2.pdf}
\end{figure}
Here the blue arrow shown in the diagram is the quantum state vector.  Specifically, if the quantum state vector is oriented an angle $\theta$ from the $z$--axis and an angle $\phi$ from the $x$--axis of the Bloch sphere if the quantum state is $\begin{bmatrix} \cos(\theta/2) \\  \sin(\theta/2) e^{i\phi}  \end{bmatrix}$ up to a global phase.    Every single qubit quantum state can be expressed in this form and thus the quantum state vector can always be visualized as a $3$D vector on the Bloch sphere.

The sphere is particularly valuable for visualizing computations on a qubit, which involves transforming the state vector of the qubit to another valid vector.  For example, a simple three-step quantum computation on one qubit might take the form:
\begin{equation}
\begin{bmatrix} 1 \\ 0 \end{bmatrix} \mapsto \begin{bmatrix} \frac{1}{\sqrt{2}}\\ \frac{1}{\sqrt{2}} \end{bmatrix} \mapsto \begin{bmatrix} \frac{1}{\sqrt{2}}\\ \frac{-1}{\sqrt{2}} \end{bmatrix} \mapsto \begin{bmatrix} 0 \\ 1 \end{bmatrix}.
\end{equation}
All of these vectors are valid quantum states because the norm of each of these vectors is $1$ as can be easily seen from the fact that $(1/\sqrt{2})^2+(1/\sqrt{2})^2=1$.  In this sense, any quantum computation can just be viewed as rotations of the quantum state vector.  

These rotations can be pictured in $3$D using the Bloch sphere representation.   The Bloch sphere can be visualized as follows:
\begin{figure}[h!]
\includegraphics[width=\linewidth]{bloch.pdf}
\end{figure}
\\\\
...
While thinking about a quantum computation as a sequence of rotations is a powerful intuition, it is challenging to use this intuition to design and describe algorithms.  \Qb from this perspective alleviates this issue by providing a language for describing such rotations.


\subsection{Single Qubit Gates}
%Now that we have discussed how data is represented in a single-qubit quantum computer we can now examine how it is processed. 
...
%Unitary transformations have the property that they norm-preserving, i.e., they do not change the norm of a vector, and maintain inner products between vectors that they act on. A matrix $U$ is unitary if $UU^\dagger=\text{I\!I}$. This implies that $U^{-1} = U^\dagger$. 
The \emph{adjoint operation} or the conjugate transpose is of crucial importance to quantum computing because it is needed to invert quantum transformations.  \Qb reflects this by providing methods to automatically compile gate sequences to for the adjoint of a unitary operation, which saves the programmer from having to hand code adjoints in many cases.


% An arbitrary matrix $U$, written as
% \begin{equation}
% U= \begin{bmatrix} \alpha &\beta \\ \gamma &\delta \end{bmatrix}
% \end{equation}
% is a unitary matrix if $UU^\dagger=\text{I\!I}$, where $U^\dagger$ and $\text{I\!I}$ are defined as
% \begin{equation}
% U^\dagger = \begin{bmatrix} \alpha^* &\gamma^* \\ \beta^* &\delta^* \end{bmatrix} \quad \mathrm{and} \quad 
% \text{I\!I} = \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix},
% \end{equation}
% where $(\cdot)^\dagger$ is called the \emph{adjoint operation} or the conjugate transpose operation.  It is an operation of crucial importance to quantum computing because it is needed to invert quantum transformations.  \Qb respects this importance by providing methods to automatically compile gate sequences to for the adjoint of a unitary operation.
% As a consequence of this definition, we have $U^\dagger = U^{-1}$, where $U^{-1}$ is the inverse of $U$. We also have
% $|\alpha|=|\delta|$, $|\beta| = |\gamma|$ and $|\alpha|^2 + |\beta|^2=1$.  
% 

There are only four functions that map one bit to one bit on a classical computer.
%In classical computing there are four single-bit transformations.  
...
\end{equation}
However, for practical reasons related to quantum error correction it is more convenient to consider a larger gate set that could be generated using $H$ and $T$ if one desired.  These gates are broken into two categories: Clifford gates and the $T$-gate.  This subdivision is useful because in many quantum error correction schemes Clifford gates are easy to implement whereas non-Clifford gates are quite costly.  The standard set of single-qubit Clifford gates that are included by default in \Qb include
\begin{align}
...
\end{align*}
Here the operations $X$, $Y$ and $Z$ are used especially frequently %(in particular in quantum simulation algorithms) 
and are named Pauli operators after their creator Wolfgang Pauli.  Together with the non-Clifford gate, the $T$-gate, these operations can be composed to approximate any unitary transformation on a single qubit.
...
%
%From this example, it may be tempting to think that qubits are mere a natural way to store a bit of information in a quantum state but this is simply not true.  In full generality a single qubit can hold a quantum state vector of the form
%\begin{equation}
%\psi = \begin{bmatrix} \sqrt{1-|\alpha|^2}\\ \alpha   \end{bmatrix},
%\end{equation} 
%where $\alpha$ is an arbitrary complex number that is at most $1$.  In other words, the quantum state held in a single qubit a two-dimensional complex vector of length $1$ (known as a unit vector).