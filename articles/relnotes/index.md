...
```
Note that this package is no longer needed for writing unit tests using the latest Q# version. 

Depending on the version of XUnit that your test project uses, you may also need to update XUnit to 2.4.1:
```
...
- The `--filter` option to `dotnet test` does not work correctly for tests written in Q#.
  Individual unit tests can be run by filtering by Trait, or use `dotnet test` at the command line to run all tests.
