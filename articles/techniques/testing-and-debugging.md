...
>>>>>>> 098c5ce29e50eb6f810b7699a5bfff9861def8f8
...
#### [Command Line / Visual Studio Code](#tab/tabid-vscode)
  ...
  > [!TIP]
  > You can figure out a qubit id by using the <xref:microsoft.quantum.primitive.message> function and passing the qubit variable in the message, for example:
  >
  > ```qsharp
...
<xref:microsoft.quantum.extensions.diagnostics.dumpregister> works like <xref:microsoft.quantum.extensions.diagnostics.dumpmachine>, except it also takes an array of qubits to limit the amount of information to only that relevant to the corresponding qubits.

As with <xref:microsoft.quantum.extensions.diagnostics.dumpmachine>, the information generated by <xref:microsoft.quantum.extensions.diagnostics.dumpregister> depends on the target machine. For the full-state quantum simulator it writes into the file the wave function up to a global phase of the quantum sub-system generated by the provided qubits in the same format as <xref:microsoft.quantum.extensions.diagnostics.dumpmachine>.  For example, take again a machine with only two qubits allocated and in the quantum state
$$
\begin{align}
