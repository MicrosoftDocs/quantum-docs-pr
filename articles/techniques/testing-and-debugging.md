...
As with <xref:microsoft.quantum.diagnostics.dumpmachine>, the information generated by @"microsoft.quantum.extensions.diagnostics.dumpregister" depends on the target machine. For the full-state quantum simulator it writes into the file the wave function up to a global phase of the quantum sub-system generated by the provided qubits in the same format as <xref:microsoft.quantum.diagnostics.dumpmachine>.  For example, take again a machine with only two qubits allocated and in the quantum state
$$
\begin{align}
