...
## Defining new operations: basics

Operations are the core of Q#.
Once declared, they can either be called from classical .NET applications, e.g., using a simulator, or by other operations within Q#.
...
Operation names must be unique within a namespace and may not conflict with type or function names.
...
The possible specializations, and how to define/call them, are detailed further below.
...
This allows for representing classical computation that interacts with quantum operations at a low level, such as in [superdense coding](https://github.com/microsoft/QuantumKatas/tree/master/SuperdenseCoding):
...
## Operation specializations: adjoint, controlled, controlled adjoint
...
## Defining operations with specializations
...
Here, we detail how to include the existence of specializations in your Q# operation declarations, hence giving them the ability to called in conjunction with the `Adjoint` and/or `Controlled` functors.
Further [below](#circumstances-for-validly-defining-specializations), we describe some of the situations in which it is either valid or not valid to declare certain specializations.
...
### Explicitly specifying specialization implementations
