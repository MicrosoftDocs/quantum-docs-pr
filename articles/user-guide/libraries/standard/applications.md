...
Yet another application ubiquitous in quantum chemistry is estimating the ground state energy of Hamiltonians representing the intermediate steps of chemical reaction. Such a scheme could, for example, rely on adiabatic state preparation to create the ground state, and then incorporate time-independent Hamiltonian simulation as a subroutine in phase estimation characterization to extract this energy with some finite error and probability of success. 

By abstracting simulation algorithms, such as the user-defined types `SimulationAlgorithm` and `TimeDependentSimulationAlgorithm`, you can conveniently incorporate their functionality into more sophisticated quantum algorithms. This motivates you to do the same for these commonly used subroutines.

Thus, you can define the convenient function
...
Thus, Shor's algorithm has had a profound impact on how security is viewed in a post-quantum world.
...
These two steps, period finding and factoring, are described here.

### Period finding

You can use the quantum Fourier transform (QFT) and phase estimation tools (for more information, see [Quantum algorithms](xref:microsoft.quantum.libraries.standard.algorithms)), to solve a classically hard computational problem called *period finding*.  The next section describes how to apply period finding to factoring.
...
To find the order using a quantum computer, use the phase estimation algorithm applied to the following unitary operator $U_a$:
...
Why is the quantum register to hold the eigenstates initialized to $\ket{1}$?
Because you do not know the order $r$ in advance, you cannot prepare $\ket{x_s}$ states directly.
Luckily, it turns out that $1/\sqrt{r} \sum\_{s=0}^{r-1} \ket{x\_s} = \ket{1}$, hence you don't need to prepare $\ket{x}$!
Instead, you can prepare a quantum register of $n$ qubits in state $\ket{1}$. 
...
To achieve $(a^nx)\text{ mod } N$, simply apply controlled-$U_{a^n}$, and calculate $a^n \text{ mod } N$ classically to plug into the quantum circuit.  
The circuits to achieve such modular arithmetic are described in the topic[Quantum algorithms](./algorithms.md#arithmetic). Specifically, a modular exponentiation circuit is required to implement the controlled-$U\_{a^i}$ operations.

While the previously described circuit corresponds to [Quantum Phase Estimation](xref:microsoft.quantum.characterization.quantumphaseestimation) and explicitly enables order finding, you can reduce the number of qubits required by either following Beauregard's method for order finding as described on [page 8 of arXiv:quant-ph/0205095v3](https://arxiv.org/pdf/quant-ph/0205095v3.pdf#page=8), or 
using one of the phase estimation routines available in [Microsoft.Quantum.Characterization](https://docs.microsoft.com/qsharp/api/qsharp/microsoft.quantum.characterization). For example, 
...
If no prime factor has been returned, proceed to the quantum routine:

5. Call the quantum period finding algorithm to calculate the order $r$ of $a \text{ mod } N$. 

Use $r$ in the classical postprocessing routine to determine the prime factors:
...
If a prime factor is not returned, then  simply repeat the algorithm from step 1.  After $n$ tries, the probability that every attempt has failed is at most $2^{-n}$.
