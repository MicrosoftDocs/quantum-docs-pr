...
We will explore this further when we discuss [types in Q#] (xref:microsoft.quantum.manual.type-model) more generally.
...
Much of this difficulty results from not having given the compiler the information it needs to recognize how the different versions of `Mapped` are related.
...
Note that the definition of `Mapped<'Input, 'Output>` looks extremely similar to the versions we wrote out before.
...
let paulis = Mapped(IntToPauli, ints);
```
...
```qsharp
function ComposedImpl(outerFn : (B -> C), innerFn : (A -> B), input : A) : C {
    return outerFn(innerFn(input));
}
function Composed(outerFn : (B -> C), innerFn : (A -> B)) : (A -> C) {
    return ComposedImpl(outerFn, innerFn, _);
}
```

Here, we must specify exactly what `A`, `B`, and `C` are, hence severely limiting the utility of our new `Composed` function.
After all, `Composed` only depends on `A`, `B`, and `C` *via* `innerFn` and `outerFn`.
As an alternative, then, we can add type parameters to `Composed` that indicate that it works for *any* `A`, `B`, and `C`, so long as these parameters match those expected by `innerFn` and `outerFn`:

```qsharp
function ComposedImpl<'A, 'B, 'C>(outerFn : ('B -> 'C), innerFn : ('A -> 'B), input : 'A) : 'C {
    return outerFn(innerFn(input));
}
function Composed<'A, 'B, 'C>(outerFn : ('B -> 'C), innerFn : ('A -> 'B)) : ('A -> 'C) {
    return ComposedImpl(outerFn, innerFn, _);
}
```
