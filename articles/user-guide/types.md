...
Details on working with expressions of each type are described in the following section, [Type expressions in Q#](xref:microsoft.quantum.guide.expressions).
...
| `Range` | sequence of integers, denoted by `start..step..stop` | `1..4` represents $\{1, 2, 3, 4\}$, `1..2..7` represents  $\{1, 3, 5, 7\}$ |  denoting `step` is optional: `start .. stop` corresponds to `start..1..stop` |
| `String` | sequence of Unicode characters, opaque to the user once created | `"Example of a string"` | used to report messages to a classical host in the case of an error or diagnostic event |
| `Unit` | the type of the empty tuple | allows only one value: `()` | used to indicate that Q# function or operation returns no information |

### Quantum-specific primitive types

- `Qubit`: represents a quantum bit or qubit.
...
- `Pauli`: represents one of the four single-qubit Pauli operators.
...
- `Result`: represents the result of a measurement.
